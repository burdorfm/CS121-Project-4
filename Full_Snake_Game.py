...
class SnakeHead(Agent):
    ...
    def __init__(self,world, xpos, ypos, direction):
        ...
        self.agility = self.length/8  #/4 for janky modern snake  #/1 for classic snake
    ...
    def update(self):
        ...
        while b.frontSeg != None:
            ...
            if abs(difVector.dx) <= self.snake.head.width/1.01 and abs(difVector.dy) <= self.snake.head.length/1.01: #2.01 for janky modern snake #2.00 for classic snake
                ...
                self.report()
                self.report()
                self.report("Snek2 has crashed into itself and is now ded! Snake1 Wins")
        ...
        while a.frontSeg != None:
            ...
            if abs(difVector.dx) <= self.snake.head.width/1.01 and abs(difVector.dy) <= self.snake.head.length/1.01: #2.01 for janky modern snake #2.00 for classic snake
                ...
                self.report()
                self.report()
                self.report("Snek2 has crashed into Snek1 and is now ded! Snek1 Wins")
                self.remove(self.snake.head2)
                self.GAME_OVER = True
            a = a.frontSeg
...
while not game.GAME_OVER:
    time.sleep(.04/60.0)  #1.0 for janky modern snake   #4.0 for classic snake
    game.update()
